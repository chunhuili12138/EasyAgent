...
点击 **AI 生成 Skill 步骤**，填写名称、编码、完整流程描述和一条可立即执行的真实试运行问题。开始生成后，输入表单会锁定，页面持续显示当前阶段、首次执行结果、尝试次数和账本事件，避免在同一任务运行期间修改输入导致界面与后台任务不一致。

- 显示 **Skill 正在生成** 时，任务仍在执行，不代表失败。
- 自动查询暂停时，任务仍可能在后台运行；点击 **继续查询状态** 获取最新结果。
- 显示 **Skill 未生成成功** 时，可根据失败信息调整描述或试运行问题，再点击 **重新生成**。
- 显示 **Skill 草稿已生成，尚未保存** 时，点击 **预览并确认保存** 进入普通 Skill 编辑窗口，检查基本信息、输入参数、执行步骤和 YAML。
- AI 生成成功只产生默认禁用的草稿，不会自动保存或启用。必须在编辑窗口底部点击 **保存**，列表中出现新记录后才算创建完成。
