...
const query = reactive({ page: 1, size: 20, runId: undefined as number | undefined, status: '', eventType: '' });

const title = computed(() => ({ log: t('automation.operations.logTitle'), failure: t('automation.operations.failureTitle'), event: t('automation.operations.eventTitle') })[props.mode]);
...
async function discardDeadLetter(row: AutomationDeadLetter) {
  await ElMessageBox.confirm(t('automation.operations.discardConfirm', { id: row.id }), t('automation.operations.discardTitle'), { type: 'warning' });
...
async function retryDeadLetter(row: AutomationDeadLetter) {
  await ElMessageBox.confirm(t('automation.operations.retryConfirm', { id: row.runId }), t('automation.operations.retryTitle'), {
    type: 'warning'
  });
...
<template>
  <div class="operations-page">
    ...
    <div class="filter-band">
      <ElInputNumber v-if="mode !== 'event'" v-model="query.runId" :min="1" :controls="false" :placeholder="t('automation.common.runId')" />
      <ElInput v-else v-model="query.eventType" clearable :placeholder="t('automation.operations.eventType')" @keyup.enter="search" />
      <ElSelect v-model="query.status" clearable :placeholder="t('automation.common.allStatus')">
        <ElOption
          v-for="value in mode === 'failure'
            ? ['OPEN', 'REPLAYED', 'DISCARDED']
            : mode === 'event'
              ? ['PENDING', 'PUBLISHED', 'RECEIVED', 'STARTED', 'PROCESSED', 'MATCHED', 'UNMATCHED', 'REJECTED']
              : ['RUNNING', 'SUCCESS', 'FAILED', 'RETRY_WAIT']"
          :key="value"
    ...
    <div class="table-band">
      ...
      <ElTable v-if="mode === 'log'" v-loading="loading" :data="records" height="100%">
        ...
        <ElTableColumn prop="errorMessage" :label="t('automation.common.errorSummary')" min-width="240" show-overflow-tooltip />
      ...
      <ElTable v-else-if="mode === 'failure'" v-loading="loading" :data="records" height="100%">
        ...
        <ElTableColumn :label="t('automation.operations.failureType')" min-width="180">
          <template #default="{ row }">
            <ElTooltip :content="row.reasonCode"><span>{{ automationFailureTypeLabel(row.reasonCode) }}</span></ElTooltip>
          </template>
        </ElTableColumn>
        <ElTableColumn :label="t('automation.common.status')" width="120">
          <template #default="{ row }">
        ...
        <ElTableColumn prop="handledBy" :label="t('automation.operations.handledBy')" width="100" />
          ...
          <template #default="{ row }">
            <template v-if="row.status === 'OPEN'">
              <ElTooltip :content="t('automation.operations.safeRetry')">
                <ElButton link type="warning" @click="retryDeadLetter(row)"><SvgIcon icon="mdi:restart" /></ElButton>
              </ElTooltip>
              <ElTooltip :content="t('automation.operations.discard')">
                <ElButton link type="danger" @click="discardDeadLetter(row)">
      ...
      <ElTable v-else v-loading="loading" :data="records" height="100%">
        <ElTableColumn prop="eventId" :label="t('automation.operations.eventId')" min-width="190" show-overflow-tooltip />
        <ElTableColumn :label="t('automation.operations.eventType')" min-width="170">
          <template #default="{ row }">
            <ElTooltip :content="row.eventType"><span>{{ automationEventTypeLabel(row.eventType) }}</span></ElTooltip>
          </template>
        </ElTableColumn>
        ...
        <ElTableColumn prop="triggerId" :label="t('automation.operations.trigger')" width="100" />
        <ElTableColumn prop="runId" :label="t('automation.common.runId')" width="100" />
        <ElTableColumn prop="payloadHash" :label="t('automation.common.payloadHash')" min-width="220" show-overflow-tooltip />
