...
const typeLabel = computed(
  () => ({
    CRON: t('automation.trigger.cron'), WEBHOOK: t('automation.trigger.webhook'),
    EVENT: t('automation.trigger.event'), APPROVAL: t('automation.trigger.approval')
  })[activeType.value]
);
...
const schemaExample = computed(() => JSON.stringify({
  type: 'object',
  required: activeType.value === 'APPROVAL' ? ['correlationKey', 'status'] : ['eventId'],
  properties: activeType.value === 'APPROVAL'
    ? { correlationKey: { type: 'string' }, status: { type: 'string' } }
    : { eventId: { type: 'string' } }
}));

async function loadWorkflows() {
  const { data, error } = await fetchWorkflows({ page: 1, size: 100, status: 'PUBLISHED' });
  workflowOptions.value = data?.records || [];
  if (error) loadError.value = error.message || t('automation.common.loadFailed');
}
...
async function rotate(row: AutomationTrigger) {
  await ElMessageBox.confirm(t('automation.trigger.rotateConfirm'), t('automation.trigger.rotateSecret'), { type: 'warning' });
  ...
  <div class="trigger-root">
    ...
    <div class="filter-band">
      <ElSelect v-model="query.workflowId" clearable filterable :placeholder="t('automation.trigger.selectWorkflow')">
        <ElOption v-for="item in workflowOptions" :key="item.id" :label="`${item.name} (${item.code})`" :value="item.id" />
      </ElSelect>
    ...
    <div class="table-band">
      ...
      <ElTable v-loading="loading" :data="records" height="100%">
        <ElTableColumn prop="name" :label="t('automation.common.name')" min-width="190" />
        <ElTableColumn prop="workflowId" :label="t('automation.common.workflow')" width="100" />
        ...
        <ElTableColumn :label="t('automation.trigger.overlapPolicy')" width="120">
          <template #default="{ row }">{{ automationOverlapPolicyLabel(row.overlapPolicy) }}</template>
        </ElTableColumn>
        <ElTableColumn prop="publicKey" :label="t('automation.trigger.publicKey')" min-width="180" show-overflow-tooltip />
        <ElTableColumn :label="t('automation.trigger.nextFireAt')" width="180">
        ...
        <ElTableColumn :label="t('automation.common.action')" width="190" fixed="right" align="center">
          <template #default="{ row }">
            ...
            <ElTooltip v-if="activeType === 'WEBHOOK' || activeType === 'APPROVAL'" :content="t('automation.trigger.rotateSecret')">
    ...
    <ElForm label-position="top">
      ...
      <template v-if="activeType === 'CRON'">
        <div class="form-grid">
          <ElFormItem :label="t('automation.trigger.cronPreset')">
            <ElSelect v-model="cronPreset" :placeholder="t('automation.trigger.cronPresetPlaceholder')" @change="applyCronPreset">
          ...
          <ElFormItem :label="t('automation.trigger.cronExpression')">
            <ElInput v-model="form.cron" :placeholder="t('automation.trigger.cronPlaceholder')" @change="syncCronPreset(form.cron)" />
          </ElFormItem>
        </div>
        <div class="form-grid">
          <ElFormItem :label="t('automation.trigger.timezone')">
            <ElSelect v-model="form.timezone" filterable allow-create :placeholder="t('automation.trigger.timezonePlaceholder')">
      ...
      <ElFormItem v-if="activeType === 'WEBHOOK' || activeType === 'APPROVAL'" :label="t('automation.trigger.signatureWindow')">
        <ElInputNumber
      ...
      <ElTabs v-model="triggerJsonTab" stretch class="trigger-json-tabs">
        <ElTabPane
          ...
          :label="activeType === 'APPROVAL' ? t('automation.trigger.callbackSchema') : t('automation.trigger.payloadSchema')"
          name="schemaJson"
        >
          <ConfigCodeEditor
        ...
        <ElTabPane v-if="activeType !== 'APPROVAL'" :label="t('automation.trigger.input')" name="inputJson">
          <ConfigCodeEditor v-model="form.inputJson" :rows="10" expected-root="object" />
        </ElTabPane>
        <ElTabPane v-if="activeType !== 'APPROVAL'" :label="t('automation.trigger.variables')" name="variablesJson">
          <ConfigCodeEditor v-model="form.variablesJson" :rows="10" expected-root="object" />
        </ElTabPane>
      </ElTabs>
    </ElForm>
    <template #footer>
    ...
    <ElDialog v-model="secretVisible" :title="t('automation.trigger.secretOnce')" width="620px" :close-on-click-modal="false">
    ...
    <template #footer><ElButton type="primary" @click="secretVisible = false">{{ t('automation.trigger.savedSecret') }}</ElButton></template>
    </ElDialog>
