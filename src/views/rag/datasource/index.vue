...
async function save() {
  ...
  try {
    ...
    const { data, error } = isEdit.value
      ? await fetchUpdateDatasource(payload.id, payload)
...
async function saveSchema() {
  ...
  try {
    ...
    const { data, error } = schemaEdit.value
      ? await fetchUpdateSchema(payload.id, payload)
      : await fetchCreateSchema(schemaDatasource.value.id, payload);
    if (error || !data) return;
