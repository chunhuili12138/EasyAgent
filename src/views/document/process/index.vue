...
const sectionCoverage = computed(() => {
  ...
  flatChunks.value
    .filter((chunk: any) => chunk.chunkType === 'child')
    .forEach((chunk: any) => {
...
const resultDuration = computed(() => {
  ...
  const seconds = Math.max(
    0,
    Math.round(
      (new Date(resultData.value.completedAt).getTime() - new Date(resultData.value.startedAt).getTime()) / 1000
    )
  );
  return seconds < 60
    ? `${seconds}${$t('page.manage.process.duration.seconds')}`
...
const stepLabels: Record<string, string> = {
  chunking: $t('page.manage.process.step.chunking'),
  labeling: $t('page.manage.process.step.labeling'),
  save: $t('page.manage.process.step.save'),
  summarizing: $t('page.manage.process.step.summarizing'),
  qa: $t('page.manage.process.step.qa')
};
...
function startPolling() {
  ...
  pollTimer = window.setInterval(() => {
    if (dataList.value.some(r => r.status === 'pending' || r.status === 'processing' || r.status === 'indexing'))
      getList();
  }, 20000);
}
...
function stepLabel(name: string) {
  return (stepLabels as any)[name] || name;
}
...
function computeDuration(row: any) {
  ...
  return s < 60
    ? s + $t('page.manage.process.duration.seconds')
    : s < 3600
      ? (s / 60).toFixed(1) + $t('page.manage.process.duration.minutes')
      : (s / 3600).toFixed(1) + $t('page.manage.process.duration.hours');
}
  ...
  try {
    ...
    if (!error) {
      ...
      for (const row of processing) {
        try {
          const { data: qs } = (await request({
            url: `/document/process/queue-status?taskId=${row.id}`,
            method: 'get'
          })) as any;
      ...
      dataList.value.some(r => r.status === 'pending' || r.status === 'processing' || r.status === 'indexing')
        ? startPolling()
        : stopPolling();
    }
  } finally {
...
function parseTagsToForm(tagsStr: string) {
  ...
  preservedTags.value = Object.fromEntries(
    internalKeys.filter(key => tags[key] !== undefined).map(key => [key, tags[key]])
  );
...
async function confirmDiscardChanges() {
  ...
  try {
    await ElMessageBox.confirm($t('page.manage.process.unsavedMessage'), $t('page.manage.process.unsavedTitle'), {
      type: 'warning',
      confirmButtonText: $t('page.manage.process.discardChanges')
    });
    cancelEditing();
    return true;
  } catch {
...
watch(tagSecurityLevel, v => {
  if (v === 'department') {
    tagForm.allow_department = tagForm.allow_department || [];
  }
...
async function handleImport(id: number) {
  try {
    await ElMessageBox.confirm($t('page.manage.process.confirmImport'), $t('page.manage.process.importConfirm'), {
      type: 'warning'
    });
    ...
    if (resultData.value?.processTaskId === id) {
      ...
      if (data) {
        resultData.value = data;
        setSelectedChunk(
          flattenChunks(data.chunks || []).find((chunk: any) => chunk.id === selectedId) || data.chunks?.[0] || null
        );
        await nextTick();
        treeRef.value?.setCurrentKey(selectedChunk.value?.id);
      }
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm($t('page.manage.process.confirmDelete'), $t('page.manage.process.deleteConfirm'), {
      type: 'warning'
    });
    const { error } = (await fetchDeleteProcess(id)) as any;
    if (error) {
      ElMessage.warning(error?.message || $t('page.manage.process.deleteFailed'));
      return;
    }
...
async function handleBatchImport() {
  if (selectedRows.value.length === 0) {
    ElMessage.warning($t('page.manage.process.selectRecords'));
    return;
  }
  try {
    await ElMessageBox.confirm(
      $t('page.manage.process.batchImportConfirm', { n: selectedRows.value.length }),
      $t('page.manage.process.batchImportTitle'),
      { type: 'warning' }
    );
    importLoading.value = true;
...
async function saveChunk() {
  ...
  try {
    await ElMessageBox.confirm($t('page.manage.process.saveChunkConfirm'), $t('common.tip'), { type: 'warning' });
  } catch {
    return;
  }
  saving.value = true;
  try {
    ...
    if (error) {
      ElMessage.warning(error?.message || $t('page.manage.process.saveFailed'));
      return;
    }
...
function filterChunkNode(value: string, data: any) {
  ...
  return [data.title, data.content, data.summary, data.keywords, parseBreadcrumb(data.breadcrumb)].some(item =>
    String(item || '')
      .toLowerCase()
      .includes(keyword)
  );
  ...
  <div class="page-container h-full">
    <ElCard class="w-full">
      <div class="mb-4 flex flex-wrap items-center gap-4">
        <ElInput
          ...
          :placeholder="$t('page.manage.process.fileName')"
          clearable
          style="width: 180px"
        />
        <ElSelect
          v-model="queryParams.status"
          :placeholder="$t('page.manage.process.status')"
          clearable
          style="width: 130px"
        >
      ...
      >
        <ElTableColumn
          type="selection"
          width="50"
          :selectable="(row: any) => row.status === 'done' || row.status === 'indexed'"
        />
        <ElTableColumn
          prop="fileName"
          :label="$t('page.manage.process.fileName')"
          min-width="200"
          align="left"
        ...
        <ElTableColumn :label="$t('page.manage.process.status')" min-width="100" align="center">
          <template #default="{ row }">
            <ElTag :type="(statusMap[row.status] || {}).tagType" size="small">
          ...
          <template #default="{ row }">
            <div v-if="(row.status === 'pending' || row.status === 'processing') && row.stepTotal">
              <div class="mb-1 flex items-center justify-center gap-1">
                <span class="text-13px text-blue-500">
                  {{
                    $t('page.manage.process.stepProgress', {
                      current: row.stepIndex || row.stepDone + 1,
                      total: row.stepTotal
                    })
                  }}
                </span>
              ...
              <ElProgress
                :percentage="Math.round((((row.stepIndex || row.stepDone + 1) - 1) / row.stepTotal) * 100)"
                :show-text="false"
                :stroke-width="4"
              />
              <div v-if="row.queueAhead > 0" class="mt-1 text-12px text-gray-400">
                {{ $t('page.manage.process.queueAhead', { n: row.queueAhead }) }}
              </div>
            </div>
            <div v-else-if="row.status === 'done' || row.status === 'indexed'" class="text-12px">
              ...
              <span class="ml-1 text-gray-500">{{ computeDuration(row) }}</span>
            </div>
            <span v-else-if="row.status === 'pending'" class="text-12px text-gray-500">
              {{ $t('page.manage.process.pending') }}
            </span>
            <span v-else-if="row.status === 'indexing'" class="text-12px text-blue-500">
              {{ $t('page.manage.process.indexing') }}
            </span>
            <span v-else-if="row.status === 'failed'" class="text-12px text-red-400">
              {{ $t('page.manage.process.failed') }}
            </span>
        ...
        <ElTableColumn :label="$t('page.manage.process.action')" min-width="200" fixed="right" align="center">
          <template #default="{ row }">
            <ElButton
              v-if="row.status === 'done' || row.status === 'indexed'"
              type="primary"
              link
              size="small"
              :loading="resultLoadingId === row.id"
              @click="showResult(row.id)"
            >
              {{ $t('page.manage.process.result') }}
            </ElButton>
            <ElButton
            ...
            >
              {{ $t('page.manage.process.importKb') }}
            </ElButton>
            <ElButton
    ...
    >
      <div v-if="resultData" class="result-dialog-body">
        <div class="stats-bar">
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.statsTotal') }}</span>
            <strong>{{ chunkStats.total }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">
              {{ $t('page.manage.process.statsParent') }}/{{ $t('page.manage.process.statsChild') }}
            </span>
            <strong>{{ chunkStats.parent }}/{{ chunkStats.child }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.tabQa') }}</span>
            <strong>{{ chunkStats.qa }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.sectionCoverage') }}</span>
            <strong>{{ sectionCoverage.covered }}/{{ sectionCoverage.total }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.averageQuality') }}</span>
            <strong>{{ chunkStats.averageQuality ?? '-' }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.llmCalls') }}</span>
            <strong>{{ resultData.llmCalls ?? '-' }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">Token</span>
            <strong>{{ resultData.tokenCost ?? '-' }}</strong>
          </div>
          <div class="stat-item">
            <span class="stat-label">{{ $t('page.manage.process.durationLabel') }}</span>
            <strong>{{ resultDuration }}</strong>
          </div>
        ...
        <div class="result-workspace">
          <aside class="chunk-tree-panel">
            <ElInput
              v-model="treeFilter"
              :placeholder="$t('page.manage.process.chunkSearch')"
              size="small"
              clearable
              class="mb-2"
            />
            ...
            >
              <template #default="{ data }">
                <div class="tree-node-row">
                  ...
                  <span class="tree-node-title" :title="data.title">
                    {{ data.title || $t('page.manage.process.noTitle') }}
                  </span>
                  <span
                    v-if="data.qualityScore != null"
                    class="quality-dot"
                    :class="[`quality-${qualityTagType(data.qualityScore)}`]"
                  >
                    {{ data.qualityScore }}
                  </span>
                </div>
            ...
            <div v-if="selectedChunk" class="editor-body">
              <div class="chunk-header">
                <div class="min-w-0">
                  ...
                  <div v-if="selectedChunk.breadcrumb" class="breadcrumb-text">
                    {{ parseBreadcrumb(selectedChunk.breadcrumb) }}
                  </div>
                </div>
              ...
              <div class="metadata-strip">
                ...
                <span>
                  <b>{{ $t('page.manage.process.indexStatus') }}</b>
                  {{ selectedChunk.esDocId ? $t('page.manage.process.indexed') : $t('page.manage.process.notIndexed') }}
                </span>
                <span v-if="selectedChunk.embeddingModel">
                  <b>{{ $t('page.manage.process.embeddingModel') }}</b>
                  {{ selectedChunk.embeddingModel }}
                </span>
              </div>
              ...
              <div v-show="editingTab === 'content'" class="tab-content">
                <div class="field-caption">
                  {{
                    selectedChunk.chunkType === 'parent'
                      ? $t('page.manage.process.parentBlock')
                      : $t('page.manage.process.childBlock')
                  }}
              ...
              <div v-show="editingTab === 'tags'" class="tags-form tab-content">
                <h4 class="mb-3 text-14px font-500">{{ $t('page.manage.process.inheritedTitle') }}</h4>
                <div class="form-grid">
                  <div class="form-row">
                ...
                <h4 class="mb-3 mt-4 text-14px font-500">{{ $t('page.manage.process.inheritableSecurityTitle') }}</h4>
                ...
                <div v-if="tagSecurityLevel !== 'public'" class="acl-subjects">
                  ...
                  <div class="flex flex-wrap gap-1">
                    <ElTag v-for="subject in aclSubjectLabels" :key="subject" size="small" effect="plain">
                      {{ subject }}
                    </ElTag>
                    <span v-if="aclSubjectLabels.length === 0" class="text-13px text-gray-400">
                      {{ $t('common.noData') }}
                    </span>
                  </div>
                </div>

                <h4 class="mb-3 mt-4 text-14px font-500">{{ $t('page.manage.process.inheritableKeywords') }}</h4>
                <div class="mb-2 flex flex-wrap gap-1">
                  <ElTag
                    v-for="(k, i) in tagForm.keywords || []"
                    :key="i"
                    :closable="isEditing"
                    size="small"
                    @close="tagForm.keywords.splice(i, 1)"
                  >
                    {{ k }}
                  </ElTag>
                  <template v-if="isEditing">
                    <ElInput
                      v-model="kwInput"
                      :placeholder="$t('page.manage.process.addKeyword')"
                      size="small"
                      style="width: 140px"
                ...
                <h4 class="mb-3 mt-4 text-14px font-500">{{ $t('page.manage.process.customTags') }}</h4>
                <div v-for="(ct, i) in customTags" :key="i" class="mb-1 flex items-center gap-1">
                  <ElInput
                    v-model="ct.key"
                    :placeholder="$t('page.manage.process.tagKey')"
                    size="small"
                    style="width: 140px"
                    :readonly="!isEditing"
                  />
                  <ElInput
                    v-model="ct.value"
                    :placeholder="$t('page.manage.process.tagValue')"
                    size="small"
                    style="width: min(260px, 100%)"
                    :readonly="!isEditing"
                  />
                  <ElButton v-if="isEditing" size="small" type="danger" plain @click="customTags.splice(i, 1)">
                    {{ $t('page.manage.process.delete') }}
                  </ElButton>
                </div>
                <ElButton v-if="isEditing" size="small" @click="customTags.push({ key: '', value: '' })">
                  + {{ $t('page.manage.process.add') }}
                </ElButton>
              </div>

              <div v-show="editingTab === 'qa'" class="tab-content">
                <div v-for="(q, i) in selectedChunk.questions || []" :key="i" class="mb-2 flex items-center gap-2">
                  <span class="min-w-24px text-12px text-gray-400">{{ Number(i) + 1 }}.</span>
                  ...
                  <ElButton
                    ...
                    type="danger"
                    plain
                    @click="selectedChunk.questions.splice(i, 1)"
                  >
                    {{ $t('page.manage.process.delete') }}
                  </ElButton>
                </div>
                <ElEmpty
                  v-if="(selectedChunk.questions || []).length === 0"
                  :description="$t('page.manage.process.noQuestions')"
                  :image-size="64"
                />
                <ElButton
                  v-if="isEditing"
                  size="small"
                  @click="selectedChunk.questions = [...(selectedChunk.questions || []), '']"
                >
                  + {{ $t('page.manage.process.addQuestion') }}
                </ElButton>
              </div>
            </div>
      ...
      <template #footer>
        <div class="result-footer">
          ...
          <div class="footer-group">
            <ElButton v-if="!isEditing" type="primary" :disabled="!selectedChunk" @click="startEditing">
              {{ $t('page.manage.process.editChunk') }}
            </ElButton>
            <template v-else>
              <ElButton type="primary" :loading="saving" :disabled="!hasUnsavedChanges" @click="saveChunk">
                {{ $t('page.manage.process.saveEdit') }}
              </ElButton>
            ...
            <ElButton
              ...
              @click="handleImport(resultData.processTaskId)"
            >
              {{
                resultData.status === 'indexed'
                  ? $t('page.manage.process.updateKb')
                  : $t('page.manage.process.importKb')
              }}
            </ElButton>
