...
import { fetchGetEsDataPage, fetchGetKbStats, fetchUnindexFile } from '@/service/api/document';
    ...
    <ElCard class="w-full">
      <div class="mb-4 flex flex-wrap items-center gap-4">
        <ElInput
          ...
          :placeholder="$t('page.manage.kb.keywordSearch')"
          clearable
          style="width: 180px"
        />
        <ElInput
          v-model="queryParams.department"
          :placeholder="$t('page.manage.kb.department')"
          clearable
          style="width: 120px"
        />
        <ElSelect
          v-model="queryParams.securityLevel"
          :placeholder="$t('page.manage.kb.securityLevel')"
          clearable
          style="width: 110px"
        >
      ...
      <div class="mb-4 flex items-center gap-2">
        <span class="ml-auto text-13px text-gray-500">
          {{ $t('page.manage.kb.totalCount') }}: {{ stats.totalChunks }}
        </span>
      </div>
      <ElTable v-loading="loading" :data="dataList" border stripe style="width: 100%">
        <ElTableColumn
          prop="file_name"
          :label="$t('page.manage.kb.fileName')"
          min-width="180"
          align="left"
        ...
        <ElTableColumn :label="$t('page.manage.kb.year')" min-width="60" align="center">
          <template #default="{ row }">
            <span class="text-13px">{{ getTagValue(row.tags, 'year') }}</span>
          </template>
        </ElTableColumn>
        <ElTableColumn :label="$t('page.manage.kb.department')" min-width="120" align="left">
          <template #default="{ row }">
            <span class="text-13px">{{ getTagValue(row.tags, 'department') }}</span>
          </template>
        </ElTableColumn>
        <ElTableColumn :label="$t('page.manage.kb.securityLevel')" min-width="90" align="center">
          <template #default="{ row }">
            <ElTag
              v-if="getTagValue(row.tags, 'security_level')"
              :type="
                getTagValue(row.tags, 'security_level') === 'user'
                  ? 'danger'
                  : getTagValue(row.tags, 'security_level') === 'post'
                    ? 'warning'
                    : getTagValue(row.tags, 'security_level') === 'department'
                      ? 'primary'
                      : 'info'
              "
              size="small"
        ...
        <ElTableColumn
          prop="summary"
          :label="$t('page.manage.kb.summary')"
          min-width="200"
          align="left"
          show-overflow-tooltip
        >
        ...
        <ElTableColumn :label="$t('page.manage.kb.action')" min-width="140" fixed="right" align="center">
          <template #default="{ row }">
            <ElButton type="primary" link size="small" @click="showDetail(row)">
              {{ $t('page.manage.kb.viewDetail') }}
            </ElButton>
            <ElButton type="warning" link size="small" @click="handleUnindex(row)">
              {{ $t('page.manage.kb.unindex') }}
            </ElButton>
          </template>
        </ElTableColumn>
      </ElTable>
    ...
    >
      <div v-if="detailRow" class="space-y-4">
        <div class="flex items-center gap-4 text-13px text-gray-500">
          <span>
            <strong>{{ $t('page.manage.kb.source') }}:</strong>
            {{ detailRow.file_name }}
          </span>
        ...
        <div class="flex flex-wrap gap-2 text-13px">
          <span v-if="getTagValue(detailRow.tags, 'year')" class="rounded bg-blue-50 px-2 py-1 text-blue-700">
            {{ $t('page.manage.kb.year') }}: {{ getTagValue(detailRow.tags, 'year') }}
          </span>
          <span v-if="getTagValue(detailRow.tags, 'department')" class="rounded bg-green-50 px-2 py-1 text-green-700">
            {{ $t('page.manage.kb.department') }}: {{ getTagValue(detailRow.tags, 'department') }}
          </span>
          <span
            v-if="getTagValue(detailRow.tags, 'security_level')"
            class="rounded px-2 py-1"
            :class="
              getTagValue(detailRow.tags, 'security_level') === 'user'
                ? 'bg-red-100 text-red-800'
                : getTagValue(detailRow.tags, 'security_level') === 'post'
                  ? 'bg-orange-100 text-orange-800'
                  : getTagValue(detailRow.tags, 'security_level') === 'department'
                    ? 'bg-yellow-100 text-yellow-800'
                    : 'bg-blue-100 text-blue-800'
            "
          >
            {{ $t('page.manage.kb.securityLevel') }}:
            {{ translateSecurityLevel(getTagValue(detailRow.tags, 'security_level')) }}
          </span>
          <span v-if="getTagValue(detailRow.tags, 'doc_type')" class="rounded bg-purple-50 px-2 py-1 text-purple-700">
            {{ getTagValue(detailRow.tags, 'doc_type') }}
          </span>
        </div>

        <div>
          <div class="mb-1 text-13px text-gray-500 font-500">{{ $t('page.manage.kb.content') }}</div>
          <div
            class="rounded bg-gray-50 p-3 text-14px leading-relaxed"
            style="
              max-height: 250px;
              overflow-y: auto;
        ...
        <div>
          <div class="mb-1 text-13px text-gray-500 font-500">{{ $t('page.manage.kb.summary') }}</div>
          <div class="rounded bg-blue-50 p-3 text-14px leading-relaxed" style="border: 1px solid #dbeafe">
            {{ detailRow.summary || '-' }}
          </div>
        </div>

        <div v-if="detailRow.qa && detailRow.qa.length > 0">
          <div class="mb-1 text-13px text-gray-500 font-500">{{ $t('page.manage.kb.questions') }}</div>
          <ul class="space-y-1">
            <li v-for="(q, i) in detailRow.qa" :key="i" class="flex items-start gap-2 text-14px">
              <span class="flex-shrink-0 text-blue-500 font-500">{{ Number(i) + 1 }}.</span>
              <span>{{ q }}</span>
            </li>
          </ul>
        </div>
