...
export const useTabStore = defineStore(SetupStoreId.Tab, () => {
  ...
  /** Clear every non-persistent tab when the authorization context changes. */
  async function clearAllTabs(navigateHome = true) {
    tabs.value = [];
    activeTabId.value = '';
    if (navigateHome && homeTab.value && router.currentRoute.value.name !== homeTab.value.routeKey) {
      await routerPush(homeTab.value.fullPath);
    }
  }
