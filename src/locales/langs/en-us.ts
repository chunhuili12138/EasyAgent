const local: App.I18n.Schema = {
  ...
  page: {
    rag_mcp: 'MCP Integration',
    mcp: {
      description: 'Streamable HTTP · read-only capabilities',
      readonlyNotice: 'This pilot exposes read-only MCP tools only. Production endpoints must use public HTTPS.',
      helpPurposeTitle: 'What this page does',
      helpPurpose:
        'Register a tenant-owned MCP Server, refresh its read-only tool catalog, and expose approved tools to the Agent through tenant isolation, tool visibility scopes, and event audit.',
      helpWorkflowTitle: 'Recommended workflow',
      helpWorkflow:
        'Register and save → validate the connection → refresh the catalog → inspect Schemas → set visibility → enable only the tools you need.',
      helpExampleTitle: 'Example configuration',
      helpExample:
        'Use a stable lowercase code such as enterprise_assets. Credentials are write-only encrypted settings and never appear in detail or audit responses.',
      helpExampleEndpoint: 'https://mcp.example.com/mcp',
      namePlaceholder: 'Example: Enterprise assets (prod)',
      codePlaceholder: 'Example: enterprise_assets',
      nameHelp:
        'Use a recognizable name that includes the business system and environment, such as “Enterprise assets (prod)”.',
      codeHelp: 'Stable English code used by the Agent catalog and audit; it cannot be changed after save.',
      endpointHelp:
        'Production requires public HTTPS; hosts resolving to private, loopback, link-local, or multicast addresses are rejected.',
      authHelp:
        'Authentication is used only for the remote MCP Server. Never put a token in the endpoint, ordinary headers, or tool descriptions.',
      credentialHelp:
        'Credentials are write-only. Leave the field blank while editing to keep the current credential; changing auth type requires the complete new credential.',
      statusHelp: 'Disabling a Server also disables its tools without deleting the cached catalog or audit history.',
      scopeHelp:
        'Visibility applies to catalog discovery, descriptions, and execution; hiding a row is not the security boundary.',
      confirmHelp:
        'Saving does not enable remote tools. Validate and refresh first, then enable tools only after confirming their read-only contract and Schema.',
      servers: 'Servers',
      noServer: 'No MCP server',
      selectServer: 'Select a server to inspect its catalog',
      validate: 'Validate',
      edit: 'Edit',
      disable: 'Disable',
      tool: 'Tool',
      ...
      enabled: 'enabled',
      events: 'Event audit',
      searchServers: 'Search name or code',
      ...
      access: 'Access',
      accessTitle: 'Configure tool access',
      departmentId: 'Department ID',
      postId: 'Post ID',
      userId: 'User ID',
      scopeSaved: 'Access scope saved',
      healthFailed: 'Health query failed',
      eventType: 'Event type',
      eventStatus: 'Event status',
      catalogStatus: 'Catalog status',
      schema: 'Schema',
      view: 'View',
      schemaTitle: 'Tool Schema',
      inputSchema: 'Input Schema',
      outputSchema: 'Output Schema',
      noSchemaHash: 'No hash',
      stepBasic: 'Basics',
      ...
      basicRequired: 'Name, code, and endpoint are required',
      endpointInvalid: 'Endpoint must be a public HTTPS URL',
      credentialRequired: 'Credentials are required for authenticated servers',
      saveFailed: 'Save failed',
      auditFailed: 'Audit query failed',
      endpointPlaceholder: 'https://mcp.example.com/mcp',
      eventColumn: 'Event',
      statusColumn: 'Status',
      errorColumn: 'Error code',
      timeColumn: 'Time',
  ...
  rag: {
    ...
    skill: {
      ...
      aiGenerate: 'Generate Skill Steps',
      aiIntro:
        'Enter a name, code, complete workflow description, and one realistic trial question. The normal Agent runs and the system reviews actual tool arguments, dependencies, results, and the final answer. If the first route lacks evidence or has a scope mismatch, the first failure is preserved and one controlled rerun is allowed before compiling a disabled Skill draft from the reviewed successful ledger.',
      aiDescription: 'Complete Description',
      aiDescriptionPlaceholder:
        'Describe inputs, execution order, required capabilities, expected results, and completion criteria. Avoid a vague one-line goal.',
      aiTrialQuery: 'Trial Question',
      aiTrialQueryPlaceholder:
        'Example: Please look up item SAMPLE-001 and tell me its name, status, price, and current inventory.',
      aiTrialQueryHint:
        'Write a realistic question that can run immediately and include the temporary values needed for this trial. They are not saved as Skill defaults.',
      aiNamePlaceholder: 'e.g. Knowledge Retrieval Answer',
      aiCodePlaceholder: 'e.g. knowledge_answer; use lowercase letters, numbers, and underscores',
      aiDescriptionTemplate:
        'Goal: describe what task the Skill should help the user complete.\nInputs: list the parameters, types, and value requirements the user will provide.\nSteps: describe which capabilities to call, in what order, and the inputs and outputs of each step.\nResult: describe the final content, language, and format to return.\nCompletion criteria: define the evidence required for success and how to handle empty or failed results.\nConstraints: use only capabilities authorized for the current tenant and avoid unrelated operations.',
      aiGenerateFailed: 'Skill generation failed',
      aiStillRunning:
        'Skill generation is still running in the background. Automatic polling paused; click “Continue checking status” for the latest result.',
      aiRunningTitle: 'Skill generation is in progress',
      aiRunningHint:
        'The form is locked while the system runs a real Agent flow and reviews tool arguments, dependencies, and results. Wait for the current stage to finish.',
      aiPollingPausedTitle: 'Generation is still running in the background',
      aiPollingPausedHint:
        'Status polling paused without marking the task as failed. Click “Continue checking status” to fetch the latest result.',
      ...
      aiActionCaptureBlocked:
        'An action Skill dry-run draft was generated, but no real request was sent. Complete manual confirmation and audit before saving or enabling it; use a read-only workflow for an automatically enableable draft.',
      aiReadyTitle: 'Skill draft generated but not saved',
      aiReadyHint:
        'Click “Preview and confirm save” to review the basic information, execution steps, and YAML. Then click “Save” at the bottom of the editor. The draft remains disabled by default.',
      aiPreviewDraft: 'Preview and confirm save',
      aiGenerating: 'Generating…',
      aiContinuePolling: 'Continue checking status',
      aiRegenerate: 'Regenerate',
      aiStages: {
        PENDING: 'Waiting',
        ANALYZING: 'Analyzing request',
        EXPLORING: 'Exploring capabilities',
        EXECUTING: 'Running normal Agent question',
        REPAIRING: 'Repairing from goal review',
        COMPILING: 'Reading ledger and compiling successful path',
        VALIDATING: 'Validating and replaying draft',
        DRAFT_READY: 'Draft ready',
        MANUAL_REPLAY_REQUIRED: 'Draft requires manual confirmation',
        TRIAL_INCOMPLETE: 'Trial incomplete',
        FAILED: 'Generation failed',
        CANCELLED: 'Cancelled'
      },
      aiStage: 'Current Stage',
      aiFirstAttempt: 'First Attempt (preserved after recovery)',
      aiFirstAttemptStatuses: {
        RUNNING: 'Running',
        FAILED: 'First attempt failed',
        SUCCEEDED: 'First attempt succeeded'
      },
      aiAttemptCount: 'Attempt Count',
      aiLedgerEvent: 'Latest Ledger Event',
      aiFailureType: 'Terminal Failure Type',
      aiRequiredFields: 'Name, code, complete description, and trial question are required',
      aiDraftReady: 'The Skill draft is ready. Preview it, trial-run it, and confirm before saving.',
      ...
      goalContract: 'Completion Check',
      requiredEvidencePlaceholder: 'The completion check is generated automatically',
      goalContractHint: 'The system evaluates completion from step dependencies, tool status, and actual results.',
      automaticGoalTitle: 'Automatic check, no manual input required',
      automaticGoalSummary: 'The Skill completes only after all {count} configured steps finish successfully.',
      automaticGoalEmptyResult:
        'A successful query with no matching records still completes normally, and the Agent reports the empty result honestly.',
      automaticGoalFailure:
        'Failed, timed out, rejected, cancelled, or missing step results never count as success; action steps still require HITL confirmation.',
      ...
      foreachMaxAttempts: 'Attempts Per Item',
      foreachContinueOnError: 'Continue After Item Failure',
      foreachBodyType: 'Body Type',
      ...
      stepSummaryParameters: '{count} parameters',
      stepSummaryLoop: 'Batch loop',
      stepSummaryTransform: 'Transform',
      stepSummaryOutputSchema: 'Output contract',
      dependencyChain: 'Dependency chains',
      stepStatusComplete: 'Configuration complete',
      stepStatusNeedsFix: 'Needs attention: {reason}',
      stepStatusUnknownParameter: 'Unknown parameter',
      stepStatusRequiredParameter: 'Required parameter missing',
      stepStatusAdvancedJson: 'Advanced JSON',
      stepStatusActionRisk: 'Action risk / HITL',
      outputSchema: 'Output Schema',
      outputSchemaHint:
        'Optional. Use the form for ordinary output contracts; it generates JSON Schema on save. Switch to Advanced JSON for complex structures. Successful primary data must satisfy the contract or the step fails. An LLM must then return valid JSON.',
      transformOutputSchemaHint:
        'Optional. Declare the downstream contract for transformed data. Use the form for ordinary nested objects and arrays; use Advanced JSON for composition or conditional rules.',
      outputSchemaPreview: 'View Generated JSON Schema',
      outputSchemaFormSummary: 'Root: {type}; fields: {fields}; required: {required}.',
      outputSchemaAdvancedSummary: 'Advanced JSON contract.',
      outputSchemaHelp:
        'Declare the JSON Schema contract for step primary data. The form supports object fields and array items recursively up to five levels and generates Draft 7 JSON. Use Advanced JSON for references, composition, or conditional rules. RAG, NL2SQL, API, built-in, LLM, and Transform steps validate it at the success boundary. For LLM, configuring a Schema parses the response as structured JSON instead of plain text. Configure output_schema inside the advanced JSON for foreach.',
      schemaForm: 'Form',
      ...
      outputSchemaDescription: 'Description',
      outputSchemaDescriptionPlaceholder: 'Describe the business meaning of the primary result',
      outputSchemaDraft7: 'Include Draft 7 $schema',
      ...
      outputSchemaFieldType: 'Type',
      outputSchemaFieldDescription: 'Business Description',
      outputSchemaFieldRequired: 'Required',
      outputSchemaFieldAdvanced: 'Advanced Properties',
      outputSchemaFieldNameRequired: 'An Output Schema field name is required',
      outputSchemaFieldDuplicate: 'Output Schema field names must be unique',
      outputSchemaEnum: 'Enum Values',
      outputSchemaEnumHint: 'Type a JSON value and press Enter, for example "urgent" or 1',
      outputSchemaFormat: 'Format',
      outputSchemaPattern: 'Pattern',
      outputSchemaMinLength: 'Minimum Length',
      outputSchemaMaxLength: 'Maximum Length',
      outputSchemaMinimum: 'Minimum',
      outputSchemaMaximum: 'Maximum',
      outputSchemaMinItems: 'Minimum Items',
      outputSchemaMaxItems: 'Maximum Items',
      outputSchemaUniqueItems: 'Unique Items',
      ...
      runParameters: 'Runtime Parameters',
      runParameterRequired: 'Parameter {key} is required',
      runParameterInvalid: 'Parameter {key} does not match its declared type',
      runBooleanTrue: 'True',
      runBooleanFalse: 'False',
      ...
      help: {
        ...
        paramsDescription:
          'After selecting an API Tool, normal mode reads its parameter Schema and lets you select names while showing type, description, required state, and enums. Unknown or duplicate names are reported before save. Enter typed literals directly; upstream bindings are grouped into source, path, cardinality, and empty-value policies. The Tool decides whether parameters go to query, path, body, or header. Keep Advanced JSON/YAML for unavailable Schemas or complex objects.',
