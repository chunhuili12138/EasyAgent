const local: App.I18n.Schema = {
  ...
  page: {
    rag_mcp: 'MCP Integration',
    mcp: {
      description: 'Streamable HTTP · read-only capabilities', readonlyNotice: 'This pilot exposes read-only MCP tools only. Production endpoints must use public HTTPS; HTTP is limited to localhost or 127.0.0.1 in local/test profiles.',
      helpPurposeTitle: 'What this page does', helpPurpose: 'Register a tenant-owned MCP Server, refresh its read-only tool catalog, and expose approved tools to the Agent through tenant isolation, tool visibility scopes, and event audit.',
      helpWorkflowTitle: 'Recommended workflow', helpWorkflow: 'Register and save → validate the connection → refresh the catalog → inspect Schemas → set visibility → enable only the tools you need.',
      helpExampleTitle: 'Example configuration', helpExample: 'Use a stable lowercase code such as enterprise_assets. Credentials are write-only encrypted settings and never appear in detail or audit responses.', helpExampleEndpoint: 'https://mcp.example.com/mcp',
      nameHelp: 'Use a recognizable name that includes the business system and environment, such as “Enterprise assets (prod)”.', codeHelp: 'Stable English code used by the Agent catalog and audit; it cannot be changed after save.', endpointHelp: 'Production requires public HTTPS and an allowlisted host; local/test profiles may use the http://127.0.0.1:9089/mcp mock.', authHelp: 'Authentication is used only for the remote MCP Server. Never put a token in the endpoint, ordinary headers, or tool descriptions.', credentialHelp: 'Credentials are write-only. Leave the field blank while editing to keep the current credential; changing auth type requires the complete new credential.', statusHelp: 'Disabling a Server also disables its tools without deleting the cached catalog or audit history.', scopeHelp: 'Visibility applies to catalog discovery, descriptions, and execution; hiding a row is not the security boundary.', confirmHelp: 'Saving does not enable remote tools. Validate and refresh first, then enable tools only after confirming their read-only contract and Schema.',
      servers: 'Servers', noServer: 'No MCP server', selectServer: 'Select a server to inspect its catalog',
  ...
  rag: {
    ...
    skill: {
      ...
      aiIntro: 'Enter a name, code, complete workflow description, and one realistic trial question. The normal Agent runs and the system reviews actual tool arguments, dependencies, results, and the final answer. If the first route lacks evidence or has a scope mismatch, the first failure is preserved and one controlled rerun is allowed before compiling a disabled Skill draft from the reviewed successful ledger.',
      aiDescription: 'Complete Description',
      aiDescriptionPlaceholder: 'Describe inputs, execution order, required capabilities, expected results, and completion criteria. Avoid a vague one-line goal.',
      aiTrialQuery: 'Trial Question',
      aiTrialQueryPlaceholder: 'Example: Please look up item SAMPLE-001 and tell me its name, status, price, and current inventory.',
      aiTrialQueryHint: 'Write a realistic question that can run immediately and include the temporary values needed for this trial. They are not saved as Skill defaults.',
      ...
      aiActionCaptureBlocked: 'An action Skill dry-run draft was generated, but no real request was sent. Complete manual confirmation and audit before saving or enabling it; use a read-only workflow for an automatically enableable draft.',
      ...
      aiStages: {
        PENDING: 'Waiting', ANALYZING: 'Analyzing request', EXPLORING: 'Exploring capabilities', EXECUTING: 'Running normal Agent question', REPAIRING: 'Repairing from goal review',
        COMPILING: 'Reading ledger and compiling successful path', VALIDATING: 'Validating and replaying draft', DRAFT_READY: 'Draft ready', MANUAL_REPLAY_REQUIRED: 'Draft requires manual confirmation',
        TRIAL_INCOMPLETE: 'Trial incomplete', FAILED: 'Generation failed', CANCELLED: 'Cancelled'
      },
      ...
      automaticGoalEmptyResult: 'A successful query with no matching records still completes normally, and the Agent reports the empty result honestly.',
      automaticGoalFailure: 'Failed, timed out, rejected, cancelled, or missing step results never count as success; action steps still require HITL confirmation.',      skillInputSource: 'Skill Parameter',
