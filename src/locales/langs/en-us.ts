const local: App.I18n.Schema = {
  ...
  page: {
    rag_mcp: 'MCP Integration',
    mcp: {
      description: 'Streamable HTTP · read-only capabilities', readonlyNotice: 'This pilot exposes read-only MCP tools only. Production endpoints must use public HTTPS.',
      ...
      namePlaceholder: 'Example: Enterprise assets (prod)', codePlaceholder: 'Example: enterprise_assets', nameHelp: 'Use a recognizable name that includes the business system and environment, such as “Enterprise assets (prod)”.', codeHelp: 'Stable English code used by the Agent catalog and audit; it cannot be changed after save.', endpointHelp: 'Production requires public HTTPS and an allowlisted host.', authHelp: 'Authentication is used only for the remote MCP Server. Never put a token in the endpoint, ordinary headers, or tool descriptions.', credentialHelp: 'Credentials are write-only. Leave the field blank while editing to keep the current credential; changing auth type requires the complete new credential.', statusHelp: 'Disabling a Server also disables its tools without deleting the cached catalog or audit history.', scopeHelp: 'Visibility applies to catalog discovery, descriptions, and execution; hiding a row is not the security boundary.', confirmHelp: 'Saving does not enable remote tools. Validate and refresh first, then enable tools only after confirming their read-only contract and Schema.',
