const local = {
  ...
  rag: {
    ...
    skill: {
      ...
      aiIntro: '输入名称、编码和完整流程描述，系统会探索当前租户可用原子工具并生成默认禁用的 Skill 草稿。',
      aiDescription: '完整描述',
      aiDescriptionPlaceholder: '说明输入参数、执行顺序、要调用的能力、需要的结果以及完成条件。不要只写一句模糊目标。',
      aiStage: '当前阶段',
      aiRequiredFields: '名称、编码和完整描述不能为空',
      ...
      goalContract: '完成条件',
      requiredEvidencePlaceholder: '每行一个必需证据字段或路径，例如 code、status',
      goalContractHint: '执行完成后系统会检查这些证据是否存在；缺失时返回可恢复结果，不会伪装成成功。',
