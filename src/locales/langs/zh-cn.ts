const local = {
  ...
  page: {
    rag_mcp: 'MCP 接入',
    mcp: {
      description: 'Streamable HTTP · 只读能力',
      readonlyNotice: '首期仅接入只读 MCP 工具。生产环境必须使用公网 HTTPS。',
      helpPurposeTitle: '功能说明',
      helpPurpose:
        '注册租户自己的 MCP Server，拉取只读工具目录，并通过统一能力目录、租户隔离、工具可见范围和事件审计提供给 Agent 使用。',
      helpWorkflowTitle: '推荐流程',
      helpWorkflow: '注册并保存 → 校验连接 → 刷新工具目录 → 查看 Schema → 配置可见范围 → 启用需要暴露的工具。',
      helpExampleTitle: '配置示例',
      helpExample: '名称可填“企业资产查询”，编码使用稳定的小写编码；认证凭据只写入加密配置，不会在详情或审计页面回显。',
      helpExampleEndpoint: 'https://mcp.example.com/mcp',
      namePlaceholder: '例如：企业资产查询（生产）',
      codePlaceholder: '例如：enterprise_assets',
      nameHelp: '用于管理员识别服务，建议包含业务系统和环境，例如“企业资产查询（生产）”。',
      codeHelp: '稳定的英文编码，供 Agent 能力目录和审计引用；保存后不可修改。',
      endpointHelp: '生产必须是公网 HTTPS；平台会拦截解析到内网、回环、链路本地或组播地址的服务。',
      authHelp: '认证凭据只用于连接远端 MCP Server。不要把 Token 写入 endpoint、普通请求头或工具描述。',
      credentialHelp: '凭据只写不读；编辑时留空表示保留原凭据，切换认证方式必须填写完整新凭据。',
      statusHelp: '停用 Server 会同时停用其工具，但不会删除历史目录和审计事件。',
      scopeHelp: '可见范围同时约束目录发现、工具描述和实际调用；页面隐藏不是安全边界。',
      confirmHelp: '保存后不会自动启用远端工具。请先校验并刷新目录，确认工具为只读且 Schema 符合预期后再启用。',
      servers: '服务',
      noServer: '暂无 MCP 服务',
      selectServer: '请选择服务查看工具目录',
      validate: '校验',
      edit: '编辑',
      disable: '停用',
      tool: '工具',
      ...
      disabledOnly: '仅已停用',
      access: '可见范围',
      accessTitle: '配置工具可见范围',
      departmentId: '部门 ID',
      postId: '岗位 ID',
      userId: '用户 ID',
      scopeSaved: '可见范围已保存',
      healthFailed: '健康状态查询失败',
      eventType: '事件类型',
      eventStatus: '事件状态',
      catalogStatus: '目录状态',
      schema: 'Schema',
      view: '查看',
      schemaTitle: '工具 Schema',
      ...
      basicRequired: '请填写名称、编码和 endpoint',
      endpointInvalid: 'endpoint 必须是公网 HTTPS 地址',
      credentialRequired: '认证方式需要填写凭据',
      saveFailed: '保存失败',
      auditFailed: '审计查询失败',
  ...
  rag: {
    ...
    skill: {
      name: '名称',
      code: '编码',
      aiCreate: 'AI 创建 Skill',
      aiGenerate: 'AI 生成 Skill 步骤',
      aiIntro:
        '输入名称、编码、完整流程描述和一条真实试运行问题。系统会让正常问答 Agent 执行，按实际工具参数、依赖、结果和最终回答审查目标是否完成；若首次路线缺少证据或范围错误，会保留首次失败并受控重执行一次，最后从通过审查的成功账本生成默认禁用的 Skill 草稿。',
      aiDescription: '完整描述',
      aiDescriptionPlaceholder: '说明输入参数、执行顺序、要调用的能力、需要的结果以及完成条件。不要只写一句模糊目标。',
      aiTrialQuery: '试运行问题',
      aiTrialQueryPlaceholder: '例如：仓库同事让我核对商品 ES-PRODUCT-LOW-STOCK-001，请告诉我名称、状态、售价和库存。',
      aiTrialQueryHint:
        '请像真实用户一样提出一次可以立即执行的问题，并提供本次所需的真实临时参数。该内容不会保存为 Skill 默认参数。',
      aiNamePlaceholder: '例如：知识库资料检索回答',
      aiCodePlaceholder: '例如：knowledge_answer，建议使用小写字母、数字和下划线',
      aiDescriptionTemplate:
        '目标：说明 Skill 要帮助用户完成什么任务。\n输入：列出用户会提供的参数、类型和取值要求。\n步骤：说明需要按什么顺序调用哪些能力，以及每一步的输入输出。\n结果：说明最终需要返回什么内容、使用什么语言和格式。\n完成条件：说明什么证据出现后才算任务完成，以及无结果或失败时如何处理。\n限制：只使用当前租户已授权的能力，不执行未授权或无关操作。',
      aiGenerateFailed: 'Skill 自动生成失败',
      aiStillRunning: 'Skill 仍在后台生成，已暂停自动查询，请点击“继续查询状态”查看最新结果',
      aiRunningTitle: 'Skill 正在生成',
      aiRunningHint: '表单已锁定。系统正在执行真实 Agent 流程并审查工具参数、依赖和结果，请等待当前阶段完成。',
      aiPollingPausedTitle: '生成仍在后台执行',
      aiPollingPausedHint: '本次状态查询已暂时停止，但任务没有判定失败。点击“继续查询状态”查看最新结果。',
      ...
      aiActionCaptureBlocked:
        '操作型 Skill 已生成 dry-run 草稿，但不会发送真实请求。保存前请完成人工确认和审计；如只需要自动生成可启用草稿，请改用只读流程。',
      aiReadyTitle: 'Skill 草稿已生成，尚未保存',
      aiReadyHint:
        '点击“预览并确认保存”检查基本信息、执行步骤和 YAML；确认无误后，在编辑窗口底部点击“保存”。草稿默认保持禁用。',
      aiPreviewDraft: '预览并确认保存',
      aiGenerating: '生成中…',
      aiContinuePolling: '继续查询状态',
      aiRegenerate: '重新生成',
      aiStages: {
        PENDING: '等待生成',
        ANALYZING: '分析需求',
        EXPLORING: '探索可用能力',
        EXECUTING: '执行正常 Agent 问答',
        REPAIRING: '根据目标审查修复执行',
        COMPILING: '读取账本并编译成功路径',
        VALIDATING: '校验并重放草稿',
        DRAFT_READY: '草稿已生成',
        MANUAL_REPLAY_REQUIRED: '草稿待人工确认',
        TRIAL_INCOMPLETE: '试运行未完成',
        FAILED: '生成失败',
        CANCELLED: '已取消'
      },
      aiStage: '当前阶段',
      aiFirstAttempt: '首次执行（修复成功也保留首次失败）',
      ...
      aiAttemptCount: '尝试次数',
      aiLedgerEvent: '最新账本事件',
      aiFailureType: '终态失败类型',
      aiRequiredFields: '名称、编码、完整描述和试运行问题不能为空',
      aiDraftReady: 'Skill 草稿已生成，请预览、试运行并确认保存',
      validationStatus: '校验状态',
      validationStatuses: {
        valid: '校验通过',
        needsRevalidation: '需要重新校验',
        invalid: '校验不通过',
        unknown: '未知状态'
      },
      version: '版本',
      ...
      inputRequired: '必填',
      addInput: '添加参数',
      inputNameDuplicate: '输入参数名不能重复',
      inputNameInvalid: '输入参数名必须以字母或下划线开头，只能包含字母、数字、下划线或连字符',
      goalContract: '完成校验',
      requiredEvidencePlaceholder: '完成校验由系统自动生成',
      goalContractHint: '系统根据步骤依赖、工具执行状态和真实返回结果自动判断是否完成。',
      automaticGoalTitle: '系统自动校验，无需填写',
      automaticGoalSummary: '当前 Skill 的 {count} 个配置步骤都成功结束后，才会判定执行完成。',
      automaticGoalEmptyResult: '查询正常完成但没有匹配记录时，仍视为成功，并由 Agent 如实说明无结果。',
      automaticGoalFailure: '步骤失败、超时、被拒绝、取消或缺少输出时不会伪装成成功；动作步骤仍必须经过 HITL 确认。',
      skillInputSource: 'Skill 参数',
      inputSelectPlaceholder: '选择 Skill 参数',
      ...
      apiParameterUnknown: '该参数不在所选工具的参数 Schema 中，请选择有效参数或切换工具。',
      apiParameterRequired: '必填',
      apiParameterOptional: '可选',
      apiParameterRequiredMissing: '所选工具的必填参数尚未全部配置，请补齐后保存。',
      ...
      foreachMaxAttempts: '单项最大尝试次数',
      foreachContinueOnError: '单项失败后继续',
      foreachBodyType: '循环体类型',
      ...
      stepSummaryParameters: '{count} 个参数',
      stepSummaryLoop: '批量循环',
      stepSummaryTransform: '数据转换',
      stepSummaryOutputSchema: '输出合同',
      dependencyChain: '依赖链',
      stepStatusComplete: '配置完整',
      stepStatusNeedsFix: '需修正：{reason}',
      stepStatusUnknownParameter: '存在未知参数',
      stepStatusRequiredParameter: '缺少必填参数',
      ...
      outputSchemaHelp:
        '为步骤主数据声明 JSON Schema 合同。对象字段和数组元素可在表单中递归维护至 5 层，页面会生成 Draft 7 JSON；引用、组合或条件规则请使用高级 JSON。RAG、NL2SQL、API、内置工具、LLM 和 Transform 都会在成功出口校验；LLM 配置后会按结构化 JSON 解析结果，而不是继续返回普通文本。foreach 请在自身高级 JSON 配置中填写 output_schema。',
      schemaForm: '表单',
      schemaJson: '高级 JSON',
      ...
      outputSchemaDescription: '整体说明',
      outputSchemaDescriptionPlaceholder: '说明该步骤主输出的业务含义',
      outputSchemaDraft7: '写入 Draft 7 $schema',
      ...
      outputSchemaFieldName: '字段名',
      outputSchemaFieldType: '类型',
      outputSchemaFieldDescription: '业务说明',
      outputSchemaFieldRequired: '必填',
      outputSchemaFieldAdvanced: '高级属性',
      outputSchemaFieldNameRequired: '输出 Schema 字段名不能为空',
      outputSchemaFieldDuplicate: '输出 Schema 字段名不能重复',
      outputSchemaEnum: '枚举值',
      outputSchemaEnumHint: '输入 JSON 值后按 Enter，例如 "urgent" 或 1',
      outputSchemaFormat: '格式',
      outputSchemaPattern: '正则',
      outputSchemaMinLength: '最小长度',
      outputSchemaMaxLength: '最大长度',
      outputSchemaMinimum: '最小值',
      outputSchemaMaximum: '最大值',
      outputSchemaMinItems: '最少元素数',
      outputSchemaMaxItems: '最多元素数',
      outputSchemaUniqueItems: '元素去重',
      outputSchemaArrayItems: '数组元素定义',
      outputSchemaChildFields: '子字段',
      outputSchemaAddChildField: '添加子字段',
      outputSchemaMaxDepthHint: '已达到表单嵌套上限（{count} 层）。更深或复杂结构请使用高级 JSON。',
      ...
      runParameters: '运行参数',
      runParameterRequired: '参数 {key} 为必填项',
      runParameterInvalid: '参数 {key} 的值不符合声明类型',
      runBooleanTrue: '是',
      runBooleanFalse: '否',
      executeActions: '执行操作',
      ...
      help: {
        ...
        paramsTitle: '参数映射',
        paramsDescription:
          '选择 API 工具后，普通模式会读取工具参数 Schema，参数名从下拉列表选择并显示类型、说明、必填和枚举；未知或重复参数会在保存前提示。固定值按参数类型填写；上游绑定收起在来源、路径、取值数量和空值策略中。参数最终进入 query、path、body 或 header 的位置由工具配置决定。无法读取 Schema 或需要复杂对象时保留高级 JSON/YAML。',
    ...
    datasource: {
      ...
      viewName: '视图名称',
      columnsMeta: '字段元数据',
      fewShotExamples: '查询示例',
      allowedFunctions: '允许函数',
      allowedFunctionsPlaceholder: '选择常用函数，或输入函数名后回车',
      sensitiveColumns: '敏感字段',
      addSchemaColumn: '添加字段',
      addSchemaExample: '添加示例',
      schemaColumnsEmpty: '尚未添加字段元数据。字段元数据为空时不会启用字段白名单。',
      schemaExamplesEmpty: '尚未添加查询示例。可按需添加经过验证的自然语言问题和只读 SQL。',
      schemaColumnName: '字段名',
      schemaColumnType: '数据类型',
      schemaColumnDescription: '业务说明',
      schemaColumnNamePlaceholder: '例如 order_no',
      schemaColumnTypePlaceholder: '例如 VARCHAR(32)',
      schemaColumnDescriptionPlaceholder: '说明字段含义、单位或枚举值',
      schemaExampleQuestion: '自然语言问题',
      schemaExampleSql: '已验证的只读 SQL',
      schemaExampleQuestionPlaceholder: '例如 查询已付款的订单',
      schemaExampleSqlPlaceholder: 'SELECT ...',
      sensitiveColumnsPlaceholder: '先添加字段元数据，再选择需要拦截的字段',
      schemaColumnRequired: '每个字段元数据都需要填写字段名、数据类型和业务说明',
      schemaColumnDuplicate: '字段名不能重复（不区分大小写）',
      schemaExampleRequired: '每个查询示例都需要填写自然语言问题和已验证的只读 SQL',
